...
def edit_patient(patient_id):
    ...
    if request.method == 'POST':
        # Update fields based on your Patient model
        patient.name = request.form.get('name')
...
def delete_doctor(doctor_id):
    ...
    try:
        ...
        # If you added the availability feature, we must clear these first
        availabilities = Availability.query.filter_by(doctor_id=doctor.id).all()
        for avail in availabilities:
            db.session.delete(avail)

        # 2. DELETE APPOINTMENTS (AND THEIR TREATMENTS)
        # We must find all appointments for this doctor
        appointments = Appointment.query.filter_by(doctor_id=doctor.id).all()
        
        for appt in appointments:
            # If an appointment has a treatment record (diagnosis/prescription), delete that first
        ...
        db.session.commit()
        flash(f'Doctor {doctor.name} and all associated records deleted.', 'success')

    except Exception as e:
        db.session.rollback()
        print(f"Error: {e}") # Check your terminal if it fails again
